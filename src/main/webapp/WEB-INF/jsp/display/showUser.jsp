...
<body>
	<div class="title ">
		<div class="c_search ">
			<div class="checktype">
				<label style="">用户名:</label> <input type="text" name="username"
					id="username">
			</div>
			<div>
		...
		<div class="c_new">
			<a href="${basePath }rest/user/toaddUser">
	...
	<table id="mytable" class="table table-hover">
		<thead>
			<tr>
				...
				<th style="background-color: #1abc9c ! important;">用户名</th>
				<th style="background-color: #1abc9c ! important;">真实姓名</th>
				<th style="background-color: #1abc9c ! important;">密码</th>
				<!-- <th style="background-color: #1abc9c ! important;">所属班组</th> -->
...
<script type="text/javascript">
	...
	function deletPro(obj) {
		...
		var url = "${basePath}rest/user/deleteUser/" + id;
		parent.layer.confirm("是否删除这个用户?", {
			btn : [ "是", "否" ]
		}, function() {
			$.ajax({
				type : "post",
	...
	$(function() {
		...
		$("#query_btn").click(function() {
			...
			QueryUser(username, 1);
		});
	});
	//查询项目详细信息并显示在页面上,里面有异步加载.
	function QueryUser(name, page) {
		var info = {
			username : name,
			PageNum : page,
		}
		$.ajax({
			...
			dataType : "json",
			data : info,
			url : "${basePath}rest/user/QueryUser",
			success : function(data) {
				...
				$(".queryById").click(function() {
					var id = $(this).data("id");
					location.href = "${basePath}rest/user/toUpdateUser/"+ id;
				});
