...
<body>
	<div class="title ">
		...
		<div class="c_new" style="width: 250px;">
			<a
				href="${basePath }rest/moduleform/toAddSmallInnovate?menuid=${menuid}">
				<button class="btn" id="newProject">新增微创新</button>
			</a> <a href="${basePath }rest/moduleform/toConfigSmallInnovate">
				<button class="btn" id="newProject">配置微创新</button>
			</a>
		</div>
	</div>
	<table id="mytable" class="table table-hover">
		<thead>
			<tr>
				...
				<th style="background-color: #1abc9c ! important;">微创新名称</th>
				<th style="background-color: #1abc9c ! important;">主要完成人</th>
				<th style="background-color: #1abc9c ! important;">成果摘要</th>
...
<script type="text/javascript">
	...
	function deletPro(obj) {
		...
		var url = "${basePath}rest/moduleform/delete";
		parent.layer.confirm("是否删除这条数据?", {
			btn : [ "是", "否" ]
		}, function() {
			$.ajax({
				type : "post",
	...
	function QueryData(pageNum, pageSize, condition) {
		...
		$
				.ajax({
					...
					dataType : "json",
					data : sendData,
					url : "${basePath}rest/moduleform/QueryData",
					success : function(data) {
						...
						if (list.length > 0) {
							for (var i = 0; i < list.length; i++) {
								html += '<tr>'
										...
										+ list[i].achievementname + '</td>'
										+ "<td style='vertical-align:middle'> "
										+ list[i].mainpersons + '</td>'
										+ "<td style='vertical-align:middle'>"
										+ list[i].achievementsummary + '</td>';
										/* + '<td style="vertical-align:middle">';
						...
						$(".queryById")
								.click(
										function() {
											var id = $(this).data("id");
											location.href = "${basePath }rest/moduleform/toUpdateSmallInnovate/"
													+ id;
										})
